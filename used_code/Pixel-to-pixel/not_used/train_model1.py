...
"""
try pytorch densnet and resnet model
"""
...
def parse_config():
    ...
    # CNN architecture
    parser.add_argument("--arch", type=str, default="u-net", help='options: FCN32s, fcn_resnet101')   #   Need to Change
    parser.add_argument("--modelname", type=str, default="test1", help='name of model')

    # training dataset
    parser.add_argument("--trainset", type=str, default="/home/x227guo/workspace/SYDE675/Pixel_to_pixel/dataset/train/")  # Need to Change 
    parser.add_argument("--testset", type=str, default="/home/x227guo/workspace/SYDE675/Pixel_to_pixel/dataset/valid/")   # Need to Change 
    ...
    parser.add_argument("--resume_epoch", type=int, default=389)      # Need to Change

    # utils
    parser.add_argument("--num_workers", type=int, default=14, help='num of threads to load data')   # Need to Change
...
class Trainer(object):
    ...
    def __init__(self, config):
        ...
        if config.arch.lower() == "fcn32s":
            from fcn import FCN32s
            from fcn import VGGNet
            vgg_model = VGGNet(pretrained=True, requires_grad=True, remove_fc=True)
            self.model = FCN32s(pretrained_net=vgg_model, n_class=1)
        elif config.arch.lower() == "fcn_resnet101":
            from fcn import fcn_resnet_p2p
            self.model = fcn_resnet_p2p(pretrained=True)
        elif config.arch.lower() in ["densenet13", "densenet"]:
            # self-made densenet ?
            # from self_densenet import DenseNet
            # self.model = DenseNet(block_config=(1, 1, 1, 1), num_classes=1, kernel_size=config.kernel_size, padding=config.padding)
    ...
    def _test_single_epoch(self, epoch):
        ...
        self.test_results.append(loss_mean)
        ...
        if os.path.isfile(ckpt):
            ...
            if self.initial_lr is not None:
                ...
                for param_group in self.optimizer.param_groups:
                    print(self.start_epoch, self.initial_lr)

...
class fcn_resnet_p2p(nn.Module):
...
class U_net(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.maxpooling(self.bn1(self.relu(self.conv1(x))))  # size = (N, 32, W/2, H/2)
        x2 = self.maxpooling(self.bn2(self.relu(self.conv2(x1)))) # size = (N, 64, W/4, H/4)
        x3 = self.maxpooling(self.bn3(self.relu(self.conv3(x2)))) # size = (N, 128, W/8, H/8)
        x4 = self.maxpooling(self.bn4(self.relu(self.conv4(x3)))) # size = (N, 256, W/16, H/16)

        x = self.relu(self.bn5(self.deconv1(x4)))                 # size = (N, 128, W/8, H/8)
        x = torch.cat((x3, x), 1)                                 # size = (N, 256, W/8, H/8)

        x = self.relu(self.bn6(self.deconv2(x)))                  # size = (N, 64, W/4, H/4)
        x = torch.cat((x2, x), 1)                                 # size = (N, 128, W/4, H/4)

        x = self.relu(self.bn7(self.deconv3(x)))                  # size = (N, 32, W/2, H/2)
        x = torch.cat((x1, x), 1)                                 # size = (N, 64, W/2, H/2)

        x = self.relu(self.bn8(self.deconv4(x)))                  # size = (N, 32, W, H)
